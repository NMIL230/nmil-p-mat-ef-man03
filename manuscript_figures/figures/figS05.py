...
FIGURE_NAME = "Figure_S05"
CSV_FILE = REPO_ROOT / "artifacts/analysis/dale_simulation/plot_data/post_run/validation/d3/dale_comparison_data.csv"


def generate(dry_run=False):
    output_dir = generated_output_dir("figS05")
    run_python_script(
        "src/nmil_dlvm/analysis/dale_simulation/plot_dale_comparison_from_csv.py",
        [
            "--csv_file", CSV_FILE,
            "--output_dir", output_dir,
            "--output_filename", "figureS05_source",
            "--metric", "kld",
            "--dlvm_label_dim", "3",
        ],
        dry_run=dry_run,
    ...
    if dry_run:
        return
    copy_pdf(output_dir / "figureS05_source.pdf", FIGURE_NAME)
