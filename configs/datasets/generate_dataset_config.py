...
from nmil_dlvm.paths import DATASET_CONFIGS_ROOT, data_dir
...
def create_config_file(DATASET):
    data_csv = pd.read_csv(data_dir(DATASET) / "all_data_time_not_normed.csv")
...
def write_py_file(variables_dict, filename):
    ...
    output_file_path = DATASET_CONFIGS_ROOT / f"{filename}.py"
...
def main():
    ...
    dataset_file = DATASET_CONFIGS_ROOT / f"{args.dataset}.py"

    if not dataset_file.exists() or args.overwrite:
