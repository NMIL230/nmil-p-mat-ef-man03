...
def get_mle_span_tasks_2params(
    ...
    while progress_fails < max_progress_fails:
        ...
        if tracker is not None:
            tracker.log(
                {
                    "best_total_log_prob": best_total_log_prob.item(),
                    "alpha": best_f[0].cpu().item(),
                    "beta": best_f[1].cpu().item(),
                }
            )
        iters+=1
        ...
        if metric_type == "binarySpan":
            ...
            if len(data) > 0:
                ...
                mle_alpha, mle_beta, _, best_prob = get_mle_beta(
                    data, max_progress_fails=mpf, tracker=tracker
                )
...
def extract_mle_parameters(mle_params_file_path=None):
    """
    ...
    """
    if mle_params_file_path is None:
        mle_params_file_path = os.fspath(data_dir(DATASET) / "all_data-best_mle_params_mpf100.pt")
