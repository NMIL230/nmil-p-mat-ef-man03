...
from nmil_dlvm.paths import (
    ...
    data_dir,
    dataset_config_path,
    ensure_dataset_runtime_dirs,
)
from configs.datasets.generate_dataset_config import create_config_file # to create config file if not exists
from nmil_dlvm.utils.data_preprocessing_utils import generate_full_matrix # to create config file if not exists
from nmil_dlvm.utils.variational_NN import variationalNN # definition of the NN model
...
PROJECT_HOME_PATH = str(PATHS_REPO_ROOT)

# Load configuration settings from the configurations.json file.
with open(PATHS_REPO_ROOT / "configurations.json", 'r') as config_file:
...
try:
    ...
    config_path = dataset_config_path(DATASET)
    if not config_path.exists():
        create_config_file(DATASET)
        config_path = dataset_config_path(DATASET)
        
    # Dynamically load the dataset module.
    spec = importlib.util.spec_from_file_location(DATASET, config_path)
    dataset_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(dataset_module)
except FileNotFoundError:
    # Exit if the module file does not exist.
    print(f"FAILED and EXITED: Module file '{module_file}' not found for dataset '{DATASET}' in repository config folders")
    sys.exit(0)
...
def ensure_dataset_ready():
    ...
    matrix_path = data_dir(DATASET) / "full_data_matrix_not_normed.pt"
    participant_ids_path = data_dir(DATASET) / "participant_ids_not_normed.csv"
    if not matrix_path.exists() or not participant_ids_path.exists():
...
def prepare_data(heldout_obs_ids=[], get_heldout_instead= False, normalize_times = True, remove_outliers = False):
    ...
    # Hold out all blacklisted sessions by default, or all except the last one for keep-outliers runs.
    default_must_holdout = OUTLIER_HELDOUT_SESSIONS
    heldout_obs_ids = heldout_obs_ids + (default_must_holdout if remove_outliers else default_must_holdout[:-1])
...
def get_model_filepath(held_out_session_ids):
    ...
    filenm = SAVED_MODELS_ROOT.relative_to(REPO_ROOT) / DATASET / f"heldout_obs{heldout_id}"
    return os.fspath(filenm)
...
def get_full_data_matrix(normalize_times=False):
    ...
    try:
        full_data_matrix = torch.load(data_dir(DATASET) / "full_data_matrix_not_normed.pt")
    except FileNotFoundError:
        print(f"Full data matrix file not found in '{data_dir(DATASET)}' folder.")
        sys.exit(0) # Exit if the file is not found.
    
    # Load the participant IDs from the saved file
    try:
        ALL_PARTICIPANT_IDS = pd.read_csv(data_dir(DATASET) / "participant_ids_not_normed.csv")["ids"].tolist()
    except FileNotFoundError:
        print(f"Participant IDs file not found in '{data_dir(DATASET)}' folder.")
        sys.exit(0)
...
def load_trained_model(latent_dim=3, model_path=None):
    ...
    if model_path is None:  # No model path provided, infer the model path using a general file pattern
        ...
        model_paths = glob.glob(
            str(SAVED_MODELS_ROOT / DATASET / "heldout_obsmulti" / filename)
        )
