...
def _format_method_label_for_table(
    method_name: str,
    method_properties: Optional[Dict[str, Dict[str, str]]] = None,
) -> str:
    """Return a human-readable method label for CSV summary tables."""
    label = method_name
    if method_properties and method_name in method_properties:
        label = method_properties[method_name].get("label", method_name)
    return label.replace(" ± SD", "").replace(" ± SE", "").strip()


def write_metric_summary_table_csv(
    df: pd.DataFrame,
    output_csv_path,
    *,
    metric: str = "kld",
    ...
    """
    Write a method-by-checkpoint summary table to CSV.

    Each output cell contains "mean +/- sd" across sessions for a method at
    the requested number of observed trials.
    """
    if checkpoints is None:
        checkpoints = [20, 80, 240]

    df = df.copy()
    if methods_filter is not None:
        df = df[df["method"].isin(methods_filter)]
        method_order = list(dict.fromkeys(methods_filter))
    else:
        method_order = sorted(df["method"].dropna().unique())

    value_col = f"{metric}_value"
    if value_col not in df.columns:
        raise ValueError(f"Expected column '{value_col}' in summary table input DataFrame")

    df = df.dropna(subset=[value_col])
    df = df[np.isfinite(df[value_col])]

    grouped = (
        df.groupby(["method", "num_tests"])[value_col]
        .agg(
    ...
    summary_lookup = {
        (row["method"], int(row["num_tests"])): (row["mean"], row["std"])
        for _, row in grouped.iterrows()
    }

    metric_label = metric.upper()
    columns = ["Method"] + [f"{metric_label} ({int(checkpoint)})" for checkpoint in checkpoints]
    rows = []
    for method_name in method_order:
        row = {
            "Method": _format_method_label_for_table(method_name, method_properties=method_properties)
        }
        for checkpoint in checkpoints:
            column_name = f"{metric_label} ({int(checkpoint)})"
            value = summary_lookup.get((method_name, int(checkpoint)))
            if value is None:
                row[column_name] = ""
                continue
