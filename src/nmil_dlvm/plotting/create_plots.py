...
from nmil_dlvm.utils.grid_search_utils import compute_loss_landscape, compute_predictions_fom_latent_points, predict_parameters_from_data, run_grid_search, generate_grid, log_prob_loss
...
import hashlib
from nmil_dlvm.paths import SAVED_MODELS_ROOT
...
def calculate_figure_params(num_figures, subfig_size=(4, 4)):
    ...
    def get_closest_factors(num):
        # Generate potential grid configurations with the constraints of max 4 rows and max 6 columns
        factors = [(i, math.ceil(num / i)) for i in range(1, min(num + 1, 5)) if math.ceil(num / i) <= 6]
        # Calculate empty slots and find the configuration with the fewest empty slots
    ...
    # Calculate the total width and height of the grid of subfigures
...
def analyze_single_model(model_path, output_path, latent_dim, specific_holdout, base_path=None,
                        use_full_primer_range=False, plot_scatters=False):
    """Analyze a single model and generate visualizations."""
    if base_path is None:
        base_path = os.fspath(REPO_ROOT)
...
def analyze_models(csv_path=None, time_stamp=None, base_path="../", latent_dim=None, 
    ...
    for idx, file_name in enumerate(file_names):
        ...
        model_path = os.fspath(
            SAVED_MODELS_ROOT / DATASET / "heldout_obsmulti" / file_name.lstrip("/")
        )
...
def visualize_latent_space_to_parameter_mapping(model, validation_meu_z, colormap='Grays', use_activation=False, show_ids=False, 
                                                plot_title=None,show_all_legends=False):
    """
    ...
    """
    # Convert validation_meu_z to numpy if it's a tensor
    ...
    if plot_title is not None:
        ...
        fig.suptitle(f'Latent Space to Parameter Mapping{" - with activation" if use_activation else " - no activation"}', fontsize=16)
    fig.patch.set_facecolor('white')
    axs = axs.flatten()
    ...
    for idx, metric in enumerate(metrics_list):
        ...
        for k, metric_idx in enumerate(fidxs):
            ...
            if metric_type == "timing" and use_activation:
                c=np.exp(c)
            scatter = axs[idx].scatter(x, y, c=c, cmap=colormap)
            axs[idx].set_title(ALL_METRICS_MOMENTS_LABEL_DICT.get(f"{metric}_param{k + 1}", f"{metric}_param{k + 1}"))

            # Plot training meu_z points (white diamonds with black edges)
            meu_z_values = model.meu_z.cpu().numpy()
            axs[idx].scatter(meu_z_values[:, 0], meu_z_values[:, 1], color='white', edgecolor='black', marker='D', s=30, label=f'Train (n = {meu_z_values.shape[0]})')
            
            # Plot validation meu_z points (red circles)
            axs[idx].scatter(validation_meu_z[:, 0], validation_meu_z[:, 1], color='red', marker='o', s=30, label=f'Validation (n = {validation_meu_z.shape[0]})')
    ...
    # make the aspect ratio of each subplot 1:1
    for ax in axs:
