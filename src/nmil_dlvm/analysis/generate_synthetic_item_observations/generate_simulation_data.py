...
DEFAULT_VALIDATION_GROUND_TRUTH = (
    ANALYSIS_ARTIFACTS_ROOT / "create_synthetic_individuals" / "synthetic_sessions_ground_truth" /
    "simulated_data" / "D3_synthetic_ground_truth_parameters_beaver-slide-5310.pt"
)
...
def resolve_ground_truth_params_path(raw_path=None):
    ...
    if raw_path is None:
        return Path(os.path.abspath(os.fspath(DEFAULT_VALIDATION_GROUND_TRUTH)))
...
def main():
    ...
    synthetic_data_dir = os.fspath(
        ensure_dir(SYNTHETIC_DATA_ROOT / dataset_name / ground_truth_params_file_name_without_extension)
    )
