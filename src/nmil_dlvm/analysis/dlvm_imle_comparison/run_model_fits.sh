...
for N in "${N_values[@]}"; do
    ...
    (
        ...
        for latent_dim in "${latent_dims[@]}"; do
            ...
            python3 "${SCRIPT_DIR}/fit_dlvm_and_imle_models_to_data.py" --fit-dlvm --latent-dim "$latent_dim" --model-id "$model_id" --max_N "$N" --synthetic_data_dir "$synthetic_data_dir" --eval_dataset_type "$eval_dataset_type" --fit-imle
