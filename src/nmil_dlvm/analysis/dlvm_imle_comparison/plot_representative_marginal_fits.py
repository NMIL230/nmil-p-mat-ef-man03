...
Usage:
    python plot_representative_marginal_fits.py \
...
from nmil_dlvm.analysis.dlvm_imle_comparison.plot_fit_error_curves_all_models import compute_rmse_per_metric, compute_kld_per_metric
from nmil_dlvm.plotting.create_marginal_fits import visualize_marginal_fits_many_methods
...
def main():
    ...
    log_file = os.fspath(LOGS_DIR / "plot_representative_marginal_fits.log")
    ...
    HARDCODED_MEDIAN_SESSION = "302run5_sim1"

    if not session_performance:
        logger.warning("Could not calculate performance for any session. Using hardcoded median session.")
        median_session_name = HARDCODED_MEDIAN_SESSION
        median_base_session = HARDCODED_MEDIAN_SESSION
    else:
    ...
    logger.info(f"Using median session: {median_session_name}")
