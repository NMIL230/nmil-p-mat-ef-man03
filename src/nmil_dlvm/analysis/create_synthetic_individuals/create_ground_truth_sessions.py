...
LOGS_DIR = ANALYSIS_ARTIFACTS_ROOT / "create_synthetic_individuals" / "logs"
DEFAULT_OUTPUT_DIR = ANALYSIS_ARTIFACTS_ROOT / "create_synthetic_individuals" / "synthetic_sessions_ground_truth"
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(LOGS_DIR / 'create_ground_truth_sessions.log')
        ]
    )
    return logging.getLogger(__name__)


def resolve_model_path(model_id, latent_dim, model_path=None):
    """
    Resolve a model path from either an explicit path or a model ID.

    Searches all saved model subdirectories for an exact filename match so
    models saved under different heldout layouts can still be found.
    """
    ...
    model_filename = f"variationalNN_relevant_only_latentdim{latent_dim}_{model_id}.pt"
    search_pattern = os.fspath(SAVED_MODELS_ROOT / DATASET / "**" / model_filename)
...
def sample_uniform_latent_points(model, num_points=88):
    """
    Sample uniform points in latent space using existing generate_grid function.
    
    Args:
        model: The loaded DLVM model
        num_points (int): Number of points to sample (default: 88)
    ...
    # If the grid is larger than requested, randomly downsample to exactly num_points.
    if len(grid_points) > num_points:
        set_seed(2*RANDOM_SEED) # just a new random seed for this function
        indices = torch.randperm(len(grid_points))[:num_points]
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default=os.fspath(DEFAULT_OUTPUT_DIR),
                       help='Output directory for results')
    parser.add_argument('--num_points', type=int, default=88,
                       help='Number of points to sample (default: 88)')
    ...
    try:
        ...
        logger.info(f"Resolved model path: {args.model_path}")
