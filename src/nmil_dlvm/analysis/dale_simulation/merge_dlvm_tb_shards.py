#!/usr/bin/env python3
"""Merge sharded DLVM_TB outputs into the cache and final comparison CSV."""
...
def configure_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("merge_dlvm_tb_shards")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Merge DLVM_TB shard outputs into cache/final CSV.")
    parser.add_argument("--manifest_csv", type=Path, required=True)
    parser.add_argument("--dim_output_dir", type=Path, required=True)
    parser.add_argument("--dlvm_tb_cache_csv", type=Path, required=True)
    ...
    logger = configure_logger(job_root / "dlvm_tb_merge.log")
    logger.info("Starting DLVM_TB shard merge")
    logger.info("Manifest: %s", args.manifest_csv)
    logger.info("Cache CSV: %s", args.dlvm_tb_cache_csv)
    ...
    for row in manifest_rows:
        ...
        shard_csv = shard_output_dir / "dlvm_tb_shard_data.csv"
    ...
    with acquire_cache_lock(args.dlvm_tb_cache_csv):
        cache_rows = load_method_data_from_csv(args.dlvm_tb_cache_csv, METHOD_NAME)
        ...
        write_rows_csv_atomic(args.dlvm_tb_cache_csv, rows)

    base_rows = read_generic_rows(args.base_csv)
    if not base_rows:
        logger.warning("Base CSV missing or empty; final CSV will contain DLVM_TB rows only")
    ...
    metadata["dlvm_tb_shard_manifest"] = str(args.manifest_csv)
    metadata["dlvm_tb_shard_rows_merged"] = count_points(shard_method_data)
    metadata["DLVM_TB_sessions"] = len(merged_rows)
    metadata["DLVM_TB_total_points"] = count_points(merged_rows)
    with args.metadata_json.open("w", encoding="utf-8") as handle:
        json.dump(metadata, handle, indent=2)

    summary = {
        "manifest_csv": str(args.manifest_csv),
        "dlvm_tb_cache_csv": str(args.dlvm_tb_cache_csv),
    ...
    summary_path = job_root / "dlvm_tb_warmup_summary.json"
    with summary_path.open("w", encoding="utf-8") as handle:
        json.dump(summary, handle, indent=2)

    logger.info("DLVM_TB cache updated: sessions=%d rows=%d", summary["final_sessions"], summary["final_points"])
    ...
    logger.info("DLVM_TB shard merge complete")
    return 0
