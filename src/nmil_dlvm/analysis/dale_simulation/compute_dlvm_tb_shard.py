#!/usr/bin/env python3
"""Compute DLVM_TB shard outputs for a subset of sessions."""
...
def configure_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("compute_dlvm_tb_shard")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compute DLVM_TB shard outputs for a subset of sessions.")
    parser.add_argument("--session_ids_file", type=Path, required=True)
    parser.add_argument("--output_dir", type=Path, required=True)
    parser.add_argument("--dale_ps0_dir", type=Path, default=None)
    parser.add_argument("--random_dir", type=Path, default=None)
    ...
    parser.add_argument("--imle_task_switch_interval", type=int, required=True)
    parser.add_argument("--workers", type=int, default=1)
    parser.add_argument("--dlvm_tb_cache_csv", type=Path, default=None)
    parser.add_argument("--base_csv", type=Path, default=None)
    parser.add_argument("--reference_mode", choices=("ps0", "synthetic_schedule"), default="ps0")
    ...
    logger.info("Starting DLVM_TB shard computation")
    ...
    logger.info("Random directory: %s", args.random_dir)
    logger.info("Model path: %s", args.model_path)
    logger.info("DLVM_TB cache: %s", args.dlvm_tb_cache_csv)
    ...
    if args.reference_mode == "synthetic_schedule":
        ...
        if args.dale_ps0_dir is None:
            raise SystemExit("--dale_ps0_dir is required when reference_mode=ps0")
    ...
    cache_method_data = load_method_data_from_csv(args.dlvm_tb_cache_csv, METHOD_NAME) if args.dlvm_tb_cache_csv else {}
    ...
    if missing_reference:
        ...
        logger.info("Computing DLVM_TB for %d uncached sessions", len(missing_reference))
        computed_method_data = compute_dlvm_tb_data(
            missing_reference,
            synthetic_data,
            ground_truth,
            None,
