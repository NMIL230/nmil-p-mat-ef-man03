#!/usr/bin/env python3  
"""
Legacy serial DALE comparison CSV builder.

This older implementation computes comparison data from DALE performance
tracking CSV files without the newer concurrent cache/locking behavior.
"""
...
def load_and_extract_predicted_params(  
    ...
    for idx, session_dir in enumerate(session_dirs, 1):  
        ...
        print(f"  → [{method_label}] Processing session {idx}/{total_sessions}: {session_id}")
            ...
            if csv_path.exists():  
                ...
                for _, row in df.iterrows():  
                    ...
                    if meu_z is not None:  
                        ...
                        # use 1-based num_tests_run for logging for consistency  
                        try:  
                        ...
                        if rmse is not None or kld is not None:  
                            iteration_data['num_tests'].append(row['num_tests_run'])  
...
def compute_imle_tb_data(dale_iteration_data: Dict, synthetic_data: Dict, ground_truth: Dict, model, metrics, parameters, logger, imle_task_switch_interval: int = 25) -> Dict:  
    imle_results = {}  
    ...
    for idx, (session_id, dale_data) in enumerate(dale_iteration_data.items(), 1):
        print(f"  → [IMLE_TB] Processing session {idx}/{total_sessions}: {session_id}")
        logger.info(f"Processing IMLE_TB session {session_id}")  
  
        synthetic_key = f"{session_id}_sim1"  
        if synthetic_key not in synthetic_data:  
            logger.warning(f"No synthetic data for session {session_id}")
            print(f"    ✗ No synthetic data for session {session_id}")
            continue  
  
        if session_id not in ground_truth:  
            logger.warning(f"No ground truth for session {session_id}")
            print(f"    ✗ No ground truth for session {session_id}")
            continue  
        ...
        for iteration in range(num_iterations):  
            ...
            if current_selected_metric is None:  
                test_to_run_next = task_sequence[current_task_index]  
                if "CorsiComplex" == test_to_run_next:  
                    current_selected_metric = random.choice(RELEVANT_METRICS_COMPLEX_SPAN)  
                elif "SimpleSpan" == test_to_run_next:  
            ...
            has_data = any(len(imle_metrics_data[metric]) > 0 for metric in RELEVANT_METRICS)  
            if not has_data:  
                continue  
            ...
            try:  
                ...
                logger.error(f"Error computing MLE for IMLE_TB {session_id} iteration {iteration}: {e}")  
                continue  
  
        if iteration_data['num_tests']:  
            imle_results[session_id] = iteration_data
            processed_count += 1
            print(f"    ✓ IMLE_TB session {session_id}: {len(iteration_data['num_tests'])} iterations stored")
            logger.info(f"IMLE_TB session {session_id}: {len(iteration_data['num_tests'])} iterations stored")  
        else:
...
def compute_imle_random_data(dale_iteration_data: Dict, synthetic_data: Dict, ground_truth: Dict, model, metrics, parameters, logger) -> Dict:  
    imle_random_results = {}  
    ...
    for idx, (session_id, dale_data) in enumerate(dale_iteration_data.items(), 1):
        print(f"  → [IMLE_Random] Processing session {idx}/{total_sessions}: {session_id}")
        logger.info(f"Processing IMLE_Random session {session_id}")  
        ...
        for iteration in range(num_iterations):  
            ...
            try:  
                ...
                logger.error(f"Error computing MLE for IMLE_Random {session_id} iteration {iteration}: {e}")  
                continue  
  
        if iteration_data['num_tests']:  
            imle_random_results[session_id] = iteration_data
            processed_count += 1
            print(f"    ✓ IMLE_Random session {session_id}: {len(iteration_data['num_tests'])} iterations stored")
            logger.info(f"IMLE_Random session {session_id}: {len(iteration_data['num_tests'])} iterations stored")  
        else:
...
def compute_dlvm_tb_data(dale_iteration_data: Dict, synthetic_data: Dict, ground_truth: Dict, model, metrics, parameters, logger, imle_task_switch_interval: int = 25) -> Dict:  
    dlvm_tb_results = {}  
    ...
    for idx, (session_id, dale_data) in enumerate(dale_iteration_data.items(), 1):
        print(f"  → [DLVM_TB] Processing session {idx}/{total_sessions}: {session_id}")
        logger.info(f"Processing DLVM_TB session {session_id}")  
        ...
        for iteration in range(num_iterations):  
            ...
            has_data = any(len(accumulated_data[metric]) > 0 for metric in RELEVANT_METRICS)  
            if not has_data:  
                continue  
            ...
            try:  
                ...
                logger.error(f"Error computing DLVM parameters for DLVM_TB {session_id} iteration {iteration}: {e}")  
                continue  
  
        if iteration_data['num_tests']:  
            dlvm_tb_results[session_id] = iteration_data
            processed_count += 1
            print(f"    ✓ DLVM_TB session {session_id}: {len(iteration_data['num_tests'])} iterations stored")
            logger.info(f"DLVM_TB session {session_id}: {len(iteration_data['num_tests'])} iterations stored")  
        else:
...
def compute_imle_dale_data(dale_dir: str, ground_truth: Dict, model, metrics, parameters, logger, session_list: List[str] = None, max_iterations: int = None) -> Dict:  
    results = {}  
    ...
    for idx, session_dir in enumerate(session_dirs, 1):  
        session_id = session_dir.name
        print(f"  → [IMLE_DALE] Processing session {idx}/{total_sessions}: {session_id}")  
  
        if is_excluded_session(session_id):  
            print(f"    ✗ Skipping excluded session '{session_id}' for IMLE_DALE")  
            continue  
  
        if session_id not in ground_truth:  
            print(f"    ✗ Skipping session {session_id} for IMLE_DALE - no ground truth available")  
            continue  
  
        data_dir = session_dir / "data"  
        if not data_dir.exists():  
            print(f"No data directory found for session {session_id}")  
            continue  
        ...
        if not data_files:  
            print(f"    ✗ No valid data files found for session {session_id}")  
            continue  
        ...
        for iteration_num, file_path in data_files:  
            try:  
                ...
                # Enforce 1-based index for num_tests for consistency (0->1, 99->100)  
                ...
                logger.debug(f"Error processing IMLE_DALE data file {file_path}: {e}")  
                continue  
  
        if iteration_data['num_tests']:  
            results[session_id] = iteration_data
            processed_count += 1
            print(f"    ✓ Loaded IMLE_DALE session {session_id}: {len(iteration_data['num_tests'])} iterations")  
        else:
...
def main():  
    ...
    configs = {
        ...
        "re100_3d": {
            ...
            "output_dir": PLOT_DATA_DIR / "re100_3d_240_try3",
            "model_path": MODEL_3D_BEAVER,
            "latent_dim": 3,
            "mle_params_file": str(GROUND_TRUTH_MLE_3D_PATH),
            "synthetic_data_path": str(SYN_DATA_3D_PATH),
        ...
        "re10_2d": {
            ...
            "output_dir": PLOT_DATA_DIR / "re10_2d_240_try3",
            "model_path": MODEL_2D_MONGOOSE,
            "latent_dim": 2,
            "mle_params_file": str(GROUND_TRUTH_MLE_2D_PATH),
            "synthetic_data_path": str(SYN_DATA_2D_PATH),
