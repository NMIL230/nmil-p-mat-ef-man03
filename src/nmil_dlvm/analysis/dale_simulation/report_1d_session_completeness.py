...
For each experiment directory under `artifacts/results/dale_runs`:
...
Usage:
  python src/nmil_dlvm/analysis/dale_simulation/report_1d_session_completeness.py \
    [--root <path_to_1d-final-now>] \
    [--gt <path_to_ground_truth_pt>] \
    [--expected 88]

Defaults:
  - root: artifacts/results/dale_runs (relative to repository root)
...
def main():
    ...
    default_root = repo_root / "artifacts" / "results" / "dale_runs"
