...
# -*- coding: utf-8 -*-
"""
Appendix-style plot script for DALE/IMLE/DLVM comparison data with reusable
styling and smart axis limits (auto tight bounds with margins + manual
override).
...
from nmil_dlvm.analysis_utils.utils_plot import (
    plot_generic_comparison,
    write_metric_summary_table_csv,
)

def build_method_properties(all_methods: List[str], dlvm_label_dim: int = 2) -> Dict[str, Dict[str, str]]:


    # Standard palette mapping (Matplotlib tab10):
    # Blue:  #1f77b4, Orange: #ff7f0e, Red: #d62728, Green: #2ca02c
    dlvm_prefix = f"DLVM-{dlvm_label_dim}"
    method_config = {
        ...
        "IMLE_Random": {
            "color": "#d62728",  # Red
            "marker": "o",
            "label": "IMLE + Random ± SD",
            "linestyle": "--",
            "sd_fill_alpha": 0.20,
        },
        # DLVM methods — solid lines, dimension-agnostic appendix labels
        "DLVM_RANDOM": {
            "color": "#d62728",  # Red
            "marker": "s",
            "label": f"{dlvm_prefix} + Random ± SD"
        },
        "DLVM_TB": {
            "color": "#2ca02c",  # Green
            "marker": "s",
            "label": f"{dlvm_prefix} + TB ± SD"
        },
        "DLVM_DALE_PS0": {
            "color": "#1f77b4",  # Blue
            "marker": "^",
            "label": f"{dlvm_prefix} + DALE + PS=0 ± SD"
        },
        "DLVM_DALE_PS1": {
            # Not specified by user; keep existing style
            "color": "#4169E1",
            "marker": "^",
            "label": f"{dlvm_prefix} + DALE + PS=1 ± SD"
        },
        "DLVM_DALE_PS2": {
            # Not specified by user; keep existing style
            "color": "#a455f3",
            "marker": "v",
            "label": f"{dlvm_prefix} + DALE + PS=2 ± SD"
        },
        "DLVM_DALE_PS4": {
            "color": "#ff7f0e",  # Orange
            "marker": "D",
            "label": f"{dlvm_prefix} + DALE + PS=4 ± SD"
        },
    }
...
def generate_individual_method_plots(
    ...
    for method in methods:
        ...
        plot_generic_comparison(
            ...
            title_fontsize=title_fontsize, title_fontweight="normal", label_fontsize=label_fontsize,
...
def generate_selected_sessions_pdf(
    ...
    *,
    ...
    # Styling for methods
    method_props = build_method_properties(sorted(set(methods)), dlvm_label_dim=dlvm_label_dim)
...
def main():
    ...
    parser.add_argument("--csv_file", type=Path,
                        default=REPO_ROOT / "artifacts/analysis/dale_simulation/plot_data/2d-final/dale_comparison_data.csv",
                        help="Path to CSV file with comparison data")
    parser.add_argument("--output_dir", type=Path,
                        default=REPO_ROOT / "artifacts/analysis/dale_simulation/plot/DALE_DLVM_Appendix_Evolution_v03",
                        help="Directory to save plots")
    parser.add_argument("--output_filename", type=str, 
                        default="DALE_DLVM_Appendix_Evolution_v03",
    ...
    parser.add_argument("--dlvm_label_dim", type=int, default=2,
                        help="Legend label dimension for DLVM prefix (e.g., 1 -> 'DLVM-1')")
    parser.add_argument("--hide_grid", dest="show_grid", action="store_false",
                        help="Disable major grid lines on the main comparison plot")
    parser.set_defaults(show_grid=True)
    # New: legend order
    parser.add_argument("--legend_order", type=str, default="none",
    ...
    parser.add_argument("--metric", type=str, default="kld", choices=["kld", "rmse"],
                        help="Metric to plot")
    parser.add_argument("--summary_table_points", type=int, nargs="*", default=[20, 80, 240],
                        help="Trial counts to summarize in a CSV table saved next to the figure")
    ...
    # Selected session export is optional; use '--sessions none' to disable it.
    ...
    parser.add_argument("--sessions_output", type=str, default="selected_sessions_appendix",
    ...
    methods = list(dict.fromkeys(args.methods)) if args.methods else sorted(df_plot['Method'].unique())
    method_props = build_method_properties(methods, dlvm_label_dim=args.dlvm_label_dim)
    ...
    summary_table_path = output_path_pdf.with_name(f"{output_path_pdf.stem}_summary_table.csv")
    write_metric_summary_table_csv(
        df=df,
        output_csv_path=summary_table_path,
        metric=args.metric,
        checkpoints=args.summary_table_points,
    ...
    plot_generic_comparison(
        ...
        title_fontsize=args.title_fontsize, title_fontweight="normal", label_fontsize=args.label_fontsize,
    ...
    if sessions_arg and sessions_arg.lower() not in {"none", "all"}:
        ...
        generate_selected_sessions_pdf(
            ...
            allow_negative_y_pad=args.allow_negative_y_pad,
            line_width=args.line_width,
            dlvm_label_dim=args.dlvm_label_dim,
        )
