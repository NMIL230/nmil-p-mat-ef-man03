...
immediately feeds it into **nmil_dlvm.cli.run_dale**. Designed to verify that
both entrypoints can interoperate end-to-end.
...
analysis_dir = os.path.join(repo_root, "src", "nmil_dlvm", "analysis", "dlvm_imle_comparison")
...
# Directories
SAVED_MODELS_DIR   = Path(repo_root) / "artifacts" / "models"
...
MODEL_3D_BEAVER = str(SAVED_MODELS_DIR / "COLL10_SIM" / "heldout_obsmulti" / "variationalNN_relevant_only_latentdim3_beaver-slide-5310.pt")
MODEL_2D_MONGOOSE = str(SAVED_MODELS_DIR / "COLL10_SIM" / "heldout_obsmulti" / "variationalNN_relevant_only_latentdim2_mongoose-dive-7464.pt")
...
def run_experiment(config: Dict, dale_script: str, analysis_dir: str) -> None:
    ...
    # Set up environment
    env = os.environ.copy()
    env["PYTHONPATH"] = os.pathsep.join([os.path.join(repo_root, "src"), env.get("PYTHONPATH", "")])
...
def main() -> None:
    ...
    dale_script = os.path.join(scripts_dir, "run_synthetic_dale.py")
