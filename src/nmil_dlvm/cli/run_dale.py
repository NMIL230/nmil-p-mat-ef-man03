...
import numpy as np
from nmil_dlvm.paths import data_dir, ensure_dataset_runtime_dirs, results_dale_runs_dir
from nmil_dlvm.analysis.dale_simulation.synthetic_dale_helpers import (
    default_synthetic_data_path,
    evaluate_model_fit_performance_for_syn_data,
    generate_primer_sequence,
    load_synthetic_data,
)
...
from nmil_dlvm.utils.grid_search_utils import get_predictions_dicts_from_latent_points
from nmil_dlvm.utils.data_distribution_utils import SUMMARIZED_METRICS_METRIC_TYPES
from nmil_dlvm.plotting.create_marginal_fits import visualize_marginal_fits_many_methods, combine_pdfs_in_folder
import json
...
def setup_run_directory(args, run_name,session_id,base_results_dir):
    ...
    Creates artifacts/results/dale_runs/[run_name]/ directory structure.
...
def generate_test_data(test_to_run_next, best_mle_params_obs, args, remaining_data=None):
    ...
    if args.use_mle_generator_for_data:
        ...
        return new_data, remaining_data
    elif args.use_synthetic_data and remaining_data is not None:
...
def initialize_model_and_prior(args, logger):
    ...
    if args.use_population_prior:
        ...
        prior_file = str(data_dir(DATASET) / f"prior_latent_pos_ld{args.latent_dim}.pt")
        ...
        meu_z = torch.nn.Parameter(popn_prior["meu_z"].cpu())
        sigma_z = torch.nn.Parameter(popn_prior["sigma_z"].cpu())
...
def load_oracle_data(args, logger):
    ...
    if args.use_synthetic_data:
        return load_synthetic_data(args, logger, RELEVANT_METRICS)

    mle_params_file = args.mle_params_file or str(data_dir(DATASET) / "all_data-best_mle_params_mpf100.pt")
...
def save_intermediate_data(run_dir, args, num_tests_run, update_w_data):
    """Persist intermediate DALE observations for synthetic workflows."""
    data_dir = os.path.join(run_dir, "data")
    update_data_file = os.path.join(
        data_dir,
        f"num_tests_run_{num_tests_run}_update_w_data_session_{args.test_session_id}_ld{args.latent_dim}.pt",
    )
    torch.save(update_w_data, update_data_file)
...
def run_active_learning(args, run_dir, logger):
    ...
    for iteration in range(args.test_budget):
        ...
        curr_posterior_beleif_latent_dist, meu_z, sigma_z, lowest_loss, test_to_run_next = perform_active_learning_update(
            ...
            always_start_from_train_mean_meu_z=args.always_start_from_train_mean_meu_z
        )

        if args.enable_primer_sequence and args.primer_sequence:
            test_to_run_next = args.primer_sequence.pop(0)

        if args.random_baseline: # select random test from the span metrics
            if args.use_synthetic_data:
                test_to_run_next = random.choice(SUMMARIZED_METRICS)
            else:
                test_to_run_next = random.choice(RELEVANT_METRICS_SPAN_CONDENSED)
        ...
        if args.save_intermediate_updates:
...
def run(args):
    ...
    base_results_dir = args.base_results_dir or str(results_dale_runs_dir())
    ...
    for session_id in args.eval_test_session_ids:
        ...
        if args.enable_primer_sequence:
            args.primer_sequence = generate_primer_sequence(
                args,
                SUMMARIZED_METRICS,
                RELEVANT_METRICS_COMPLEX_SPAN,
                RELEVANT_METRICS_SIMPLE_SPAN,
            )
        else:
            args.primer_sequence = []
        
        try:
            logger.info(f"Sending {args} to `run_active_learning`")
...
def main(argv=None):
    ...
    parser.add_argument('--base_results_dir', type=str, default=str(results_dale_runs_dir()))
    ...
    parser.add_argument('--test_budget', type=int, default=-1)  # -1 means use COLL10 data count
    ...
    parser.add_argument('--wandb_entity', default="lsnn")
    parser.add_argument('--wandb_project_name', default="mat-dale-nn")
    ...
    parser.add_argument('--use_synthetic_data', type=str2bool, default=False)
    parser.add_argument(
        '--synthetic_data_file',
        type=str,
        default=str(default_synthetic_data_path(repo_root, DATASET)),
    )
    parser.add_argument('--mle_params_file', type=str, default=None)
    parser.add_argument(
        "--sim_index",
        type=int,
        default=1,
        help="Synthetic replicate index, used to resolve '<session>_simN' keys.",
    )
    
    # Visualization parameters
    parser.add_argument('--create_marginal_fits_visualizations', type=str2bool, default=True)
    parser.add_argument('--visualization_frequency', type=int, default=1)  # Create visualization every N tests
    parser.add_argument('--save_intermediate_updates', type=str2bool, default=False)

    # Active learning parameters
    parser.add_argument('--num_restarts', type=int, default=100)    
    ...
    parser.add_argument('--enable_primer_sequence', type=str2bool, default=False)
    parser.add_argument('--primer_sequence_task_repetitions', type=int, default=4)
    parser.add_argument('--always_start_from_train_mean_meu_z', action='store_true', default=False, help="Whether to start the active learning update from the mean of the trained model")
    ...
    ensure_dataset_runtime_dirs(DATASET)
