...
from nmil_dlvm.utils.grid_search_utils import compute_loss_landscape
...
from nmil_dlvm.utils.grid_search_utils import run_grid_search
...
def train_model(args):
    ...
    mean_log_probs = torch.load(data_dir(DATASET) / "mean_log_probabilities.pt", weights_only=False)
    std_log_probs = torch.load(data_dir(DATASET) / "std_log_probabilities.pt", weights_only=False)

    filenm = REPO_ROOT / get_model_filepath(args.held_out_session_ids)
    filenm.mkdir(parents=True, exist_ok=True)
    ...
    data_dict, metrics, participant_ids = prepare_data(
        heldout_obs_ids=args.held_out_session_ids,
    ...
    test_indices = []
    if heldout_size > 0:
        val_subset, test_subset = random_split(range(heldout_size), [val_size, test_size])
    ...
    use_validation = len(vals_update_w_data_list) > 0
    if not use_validation:
        logging.info("No validation sessions available; skipping validation loss computation.")
    ...
    for e in range(args.n_epochs):
        ...
        if use_validation:
            ...
            val_loss_list.append(final_val_loss)  # Append the validation loss

            if args.verbose:
                logging.info(f"epoch: {e + 1}, train_loss: {train_loss.item():.4f}, kld: {kld.item():.4f},param_prior_prob: {prior_param_loss.item():.4f}, val_loss: {final_val_loss:.4f}")
            ...
            best_model = copy.deepcopy(model.state_dict())
            if args.verbose:
                logging.info(f"epoch: {e + 1}, train_loss: {train_loss.item():.4f}, kld: {kld.item():.4f},param_prior_prob: {prior_param_loss.item():.4f}, val_loss: skipped")
    ...
    csv_file_path = model_training_analysis_dir(DATASET) / f"{args.run_mode}s_data.csv"
    ...
    new_record = {
        ...
        'final_avg_log_prob_val': final_val_loss,
        'total_count': N,
        'final_kld': kld.item(),
...
def save_loss_plot(loss_list, val_loss_list, training_params):
    ...
    folder_path = visualization_outputs_dir(DATASET, model_id)
    folder_path.mkdir(parents=True, exist_ok=True)

    file_name_natural = str(folder_path / "training_loss_natural")
    file_name_log = str(folder_path / "training_loss_log")
...
def build_parser():
    ...
    parser.add_argument(
        "--keep_outliers",
        action="store_true",
        help="Keep blacklisted outlier sessions in the training set.",
    )
    return parser
...
def main(argv=None):
    ...
    args.vis_output_dir = str(visualization_outputs_dir(DATASET, args.model_id))
    os.makedirs(args.vis_output_dir, exist_ok=True)

    log_file = os.path.join(args.vis_output_dir, f"{args.model_id}_training_log.txt")
